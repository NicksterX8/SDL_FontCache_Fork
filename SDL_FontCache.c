...
// The number of fonts that have been created but not yet freed
...
inline FC_Scale FC_MakeScale(float x, float y)
{
    return (FC_Scale){x, y};
}
...
FC_Effect FC_MakeEffect(FC_HAlignEnum horizontalAlignment, FC_VAlignEnum verticalAlignment,
    FC_Scale scale, SDL_Color color)
{
    FC_Effect e;

    e.horizontal_align = horizontalAlignment;
    e.vertical_align = verticalAlignment;
    e.scale = scale;
    e.color = color;
...
#define MAX_CACHED_TEXT_LENGTH 100

typedef struct FC_TextCache
{
    char text[MAX_CACHED_TEXT_LENGTH]; // Does not hold more than 100 character long strings
    // we need to keep certain render options around that change how the text would be rendered
    // FC_TextRenderOptions options;
    FC_HAlignEnum hAlign;
...
struct FC_Font
{
    ...
    SDL_Color glyph_cache_color;
    SDL_Texture** glyph_cache;

    // the cache for holding previously rendered text with textures
    FC_TextCache* text_cache;
    // number of text textures currently stored
    int text_cache_count;
    // the incrementing index for the internal queue managing the text cache
    int text_cache_queue_index;

    // maximum number of text caches stored
    int text_cache_max_size;
...
static int FC_Int_RenderCenter(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text);
static int FC_Int_RenderRight(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text);
...
inline FC_Rect FC_MakeRect(float x, float y, float w, float h) {
...
FC_Rect FC_DefaultRenderCallback(SDL_Texture* src, FC_Rect* srcrect, FC_Target* dest, float x, float y, float xscale, float yscale)
{
...
static void FC_VoidRenderCallback(SDL_Texture* src, FC_Rect* srcrect, FC_Target* dest, float x, float y, float xscale, float yscale)
{
    int flip = SDL_FLIP_NONE;
    flip |= SDL_FLIP_HORIZONTAL * (xscale < 0);
    xscale = (xscale < 0) ? -xscale : xscale; 

    flip |= SDL_FLIP_VERTICAL * (yscale < 0);
    yscale = (yscale < 0) ? -yscale : yscale;

    SDL_Rect dr = {(int)x, (int)y, (int)(xscale*srcrect->w), (int)(yscale*srcrect->h)};
    SDL_RenderCopyEx(dest, src, srcrect, &dr, 0, NULL, (SDL_RendererFlip)flip);
}

static FC_Rect (*fc_render_callback)(SDL_Texture* src, FC_Rect* srcrect, FC_Target* dest, float x, float y, float xscale, float yscale) = &FC_DefaultRenderCallback;

void FC_SetRenderCallback(FC_Rect (*callback)(SDL_Texture* src, FC_Rect* srcrect, FC_Target* dest, float x, float y, float xscale, float yscale))
{
    if (callback == NULL) {
...
int FC_FlushTextCache(FC_Font* font)
{
    for (int i = 0; i < font->text_cache_count; i++)
    {
        // setting these both to null is theoretically not needed, but do it just in case
        font->text_cache[i].texture = NULL;
        font->text_cache[i].text[0] = '\0';
    }
    font->text_cache_count = 0;
    return 0;
}
...
void FC_SetTabWidth(FC_Font* font, unsigned int width_in_spaces)
{
    font->tab_width = width_in_spaces;
    FC_FlushTextCache(font);
}

// Constructors

// Are we responsible for shutting down and in general managing TTF?
// Default to no, Set to 1 when we init TTF.
static int manage_ttf = 0;

int FC_Init(int manageTTF)
{
    if (manageTTF) {
        TTF_Init();
    }
    if (!TTF_WasInit()) {
        FC_Log("FontCache Error: TTF was not initialized before FC_Init was called. Consider calling FC_Init with a value of 1 to automatically initialize TTF.");
        return -1;
    }
    fc_buffer = (char*)malloc(fc_buffer_size);
...
void FC_Quit()
{
    ...
    // Make sure all fonts were freed
    if (NUM_EXISTING_FONTS > 0)
    {
        // Help users to see memory leaks
        FC_Log("Warning: FontCache was quit without freeing some fonts");
        free(ASCII_STRING);
        ASCII_STRING = NULL;
...
#define FC_DEFAULT_TEXT_CACHE_MAX_SIZE 52428800 // 50 megabites
#define FC_DEFAULT_TEXT_CACHE_COUNT 0

static void FC_InitFont(FC_Font* font)
{
    if (font == NULL) return;
    font->renderer = NULL;
    ...
    if (font->text_cache == NULL) {
        font->text_cache_count = 0;
        font->text_cache_max_size = 1;
        font->text_cache = malloc(sizeof(FC_TextCache) * FC_DEFAULT_TEXT_CACHE_COUNT);
    } else {
        FC_FlushTextCache(font);
    }
...
static Uint8 FC_GrowGlyphCache(FC_Font* font)
{
...

    {
        ...
        if (prev_target) {
            prev_clip_enabled = SDL_RenderIsClipEnabled(font->renderer);
...
static FC_Rect FC_GetDestinationRectLeft(FC_Font* font, const char* text)
{
    const char* c = text;
    FC_Rect srcRect;
    FC_Rect dstRect;
    FC_Rect dirtyRect = FC_MakeRect(0, 0, 0, 0);
    ...
    if (font == NULL)
        return dirtyRect;
    if (c == NULL || font->glyph_cache_count == 0)
        return dirtyRect;

    destH = font->height;
    destLineSpacing = font->lineSpacing;
    destLetterSpacing = font->letterSpacing;
...
static FC_Rect FC_GetDestinationRectAlignOffset(FC_Font* font, float textWidthOffset, const char* text)
{
    int y = 0;
    FC_Rect result = {0, 0, 0, 0};
    ...
    for(c = str; *c != '\0';)
    {
        if(*c == '\n')
        {
            *c = '\0';
            ...
            FC_Rect leftRect = FC_GetDestinationRectLeft(font, str);
            leftRect.x -= FC_GetWidth(font, "%s", str)*textWidthOffset;
            leftRect.y += y;
            result = FC_RectUnion(leftRect, result);
            *c = '\n';
            c++;
            str = c;
            y += font->height;
...
static_inline FC_Rect FC_GetDestinationRectRight(FC_Font* font, const char* text)
{
    return FC_GetDestinationRectAlignOffset(font, 1.0f, text);
}
static_inline FC_Rect FC_GetDestinationRectCenter(FC_Font* font, const char* text)
{
    return FC_GetDestinationRectAlignOffset(font, 2.0f, text);
}
static FC_Rect FC_GetDestinationRect(FC_Font* font, FC_HAlignEnum align, const char* text)
{
    switch (align)
    {
        case FC_HALIGN_LEFT:
            return FC_GetDestinationRectLeft(font, text);
        case FC_HALIGN_RIGHT:
            return FC_GetDestinationRectRight(font, text);
        case FC_HALIGN_CENTER:
            return FC_GetDestinationRectCenter(font, text);
    }
    return FC_MakeRect(0,0,0,0);
}

static int FC_Int_RenderLeft(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text)
{
    const char* c = text;
    FC_Rect srcRect;
    ...
    if (font == NULL || dest == NULL) return -1;

    destH = font->height * scale.y;
    destLineSpacing = font->lineSpacing*scale.y;
    destLetterSpacing = font->letterSpacing*scale.x;

    if (c == NULL || font->glyph_cache_count == 0) return -1;
    ...
    for(; *c != '\0'; c++)
    {
        ...
        FC_VoidRenderCallback(FC_GetGlyphCacheLevel(font, glyph.cache_level), &srcRect, dest, destX, destY, scale.x, scale.y);
...
void FC_Draw(FC_Font* font, FC_Target* dest, float x, float y, const char* formatted_text, ...)
{
    if (formatted_text == NULL || font == NULL) return;

    FC_EXTRACT_VARARGS(fc_buffer, formatted_text);

    set_color_for_all_caches(font, font->default_color);

    FC_RenderLeft(font, dest, x, y, FC_MakeScale(1,1), fc_buffer);
}
...
static void FC_RenderAlign(FC_Font* font, FC_Target* dest, float x, float y, int width, FC_Scale scale, FC_HAlignEnum align, const char* text)
{
    switch(align)
    { 
        case FC_HALIGN_LEFT:
...
static void FC_DrawColumnFromBuffer(FC_Font* font, FC_Target* dest, FC_Rect box, int* total_height, FC_Scale scale, FC_HAlignEnum align)
{
    int y = box.y;
    FC_StringList *ls, *iter;
...
FC_Rect FC_DrawBox(FC_Font* font, FC_Target* dest, FC_Rect box, const char* formatted_text, ...)
{
    ...
    useClip = SDL_RenderIsClipEnabled(dest);
    ...
    FC_DrawColumnFromBuffer(font, dest, box, NULL, FC_MakeScale(1,1), FC_HALIGN_LEFT);
...
FC_Rect FC_DrawBoxAlign(FC_Font* font, FC_Target* dest, FC_Rect box, FC_HAlignEnum align, const char* formatted_text, ...)
{
    Uint8 useClip;
    if(formatted_text == NULL || font == NULL)
        return FC_MakeRect(box.x, box.y, 0, 0);
...
FC_Rect FC_DrawBoxScale(FC_Font* font, FC_Target* dest, FC_Rect box, FC_Scale scale, const char* formatted_text, ...)
{
    ...
    FC_DrawColumnFromBuffer(font, dest, box, NULL, scale, FC_HALIGN_LEFT);
...
FC_Rect FC_DrawBoxEffect(FC_Font* font, FC_Target* dest, FC_Rect box, FC_Effect effect, const char* formatted_text, ...)
{
    ...
    FC_DrawColumnFromBuffer(font, dest, box, NULL, effect.scale, effect.horizontal_align);
...
FC_Rect FC_DrawColumn(FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, const char* formatted_text, ...)
{
    ...
    FC_DrawColumnFromBuffer(font, dest, box, &total_height, FC_MakeScale(1,1), FC_HALIGN_LEFT);

    return FC_MakeRect(box.x, box.y, width, total_height);
}

FC_Rect FC_DrawColumnAlign(FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, FC_HAlignEnum align, const char* formatted_text, ...)
{
    FC_Rect box = {x, y, width, 0};
    int total_height;
...
FC_Rect FC_DrawColumnScale(FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, FC_Scale scale, const char* formatted_text, ...)
{
    ...
    FC_DrawColumnFromBuffer(font, dest, box, &total_height, scale, FC_HALIGN_LEFT);
...
FC_Rect FC_DrawColumnEffect(FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, FC_Effect effect, const char* formatted_text, ...)
{
    ...
    switch(effect.horizontal_align)
    {
    case FC_HALIGN_CENTER:
        box.x -= width/2;
        break;
    ...
    FC_DrawColumnFromBuffer(font, dest, box, &total_height, effect.scale, effect.horizontal_align);

    return FC_MakeRect(box.x, box.y, width, total_height);
}

/*
* Render text with an offset of the text output width.
* Example: 0.5 would align the text to the center,
* and 1 would align it to the right
*/
static FC_Rect FC_RenderAlignOffset(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, float textWidthOffset, const char* text)
{
    ...
    for(c = str; *c != '\0';)
    {
        ...
        {
            *c = '\0';
            result = FC_RectUnion(FC_RenderLeft(font, dest, x - scale.x*FC_GetWidth(font, "%s", str)*textWidthOffset, y, scale, str), result);
            *c = '\n';
            c++;
            str = c;
...
* and 1 would align it to the right
*/
static int FC_Int_RenderAlignOffset(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, float textWidthOffset, const char* text)
{
    if(text == NULL || font == NULL)
        return -1;
    ...
    for(c = str; *c != '\0';)
    {
        ...
        {
            *c = '\0';
            FC_Int_RenderLeft(font, dest, x - scale.x*FC_GetWidth(font, "%s", str)*textWidthOffset, y, scale, str);
            *c = '\n';
            c++;
            str = c;
            y += scale.y*font->height;
    ...
    FC_RenderLeft(font, dest, x - scale.x*FC_GetWidth(font, "%s", str)*textWidthOffset, y, scale, str);

    free(del);
    return 0;
}

static_inline FC_Rect FC_RenderCenter(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text)
{
    return FC_RenderAlignOffset(font, dest, x, y, scale, 0.5f, text);
}

static_inline FC_Rect FC_RenderRight(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text)
{
    return FC_RenderAlignOffset(font, dest, x, y, scale, 1.0f, text);
}

static_inline int FC_Int_RenderCenter(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text)
{
    return FC_Int_RenderAlignOffset(font, dest, x, y, scale, 0.5f, text);
}

static_inline int FC_Int_RenderRight(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text)
{
    return FC_Int_RenderAlignOffset(font, dest, x, y, scale, 1.0f, text);
}
...
Uint16 FC_GetBufferHeight(FC_Font* font) {
    if(font == NULL)
        return 0;
    ...
    //   Actual height of letter region + line spacing
    return font->height*numLines + font->lineSpacing*(numLines - 1);
}
...
Uint16 FC_GetBufferWidth(FC_Font* font)
{
    if (font == NULL) return 0;
...
FC_Rect FC_GetBounds(FC_Font* font, float x, float y, FC_HAlignEnum align, FC_Scale scale, const char* formatted_text, ...)
{
    FC_Rect result = {x, y, 0, 0};
...
Uint16 FC_GetPositionFromOffset(FC_Font* font, float x, float y, int column_width, FC_HAlignEnum align, const char* formatted_text, ...)
{
    FC_StringList *ls, *iter;
    Uint8 done = 0;
...
#define FC_NULLCHECK_FONT(font, errorValue) { if((font) == NULL) { SDL_Log("Font passed to FC function was NULL, returning."); return (errorValue); } }

void FC_SetFilterMode(FC_Font* font, FC_FilterEnum filter)
{
    FC_NULLCHECK_FONT(font, (void)0);
    font->filter = filter;
    FC_FlushTextCache(font);
}
...
FC_Rect FC_DrawBufferUnsafe(FC_Font* font, FC_Target* dest, float x, float y, 
    FC_HAlignEnum alignHorizontal, FC_VAlignEnum alignVertical, 
    FC_Scale scale, SDL_Color color)
{
    ...
    int height = FC_GetBufferHeight(font);
    switch (alignVertical)
    {
        case FC_VALIGN_TOP:
            break;
        case FC_VALIGN_MIDDLE:
            // FC_GetBufferHeight's value will not be relative to scale
            dstY -= height/2 * scale.y;
            break;
        case FC_VALIGN_BOTTOM:
            dstY -= height * scale.y;
            break;
    }
    ...
    {
        case FC_HALIGN_LEFT:
            result = FC_RenderLeft(font, dest, dstX, dstY, scale, fc_buffer);
            break;
        case FC_HALIGN_CENTER:
            result = FC_RenderCenter(font, dest, dstX, dstY, scale, fc_buffer);
            break;
        case FC_HALIGN_RIGHT:
            result = FC_RenderRight(font, dest, dstX, dstY, scale, fc_buffer);
            break;
        default:
...
int FC_Int_DrawBufferUnsafe(FC_Font* font, FC_Target* dest, float x, float y, 
    FC_HAlignEnum alignHorizontal, FC_VAlignEnum alignVertical, 
    FC_Scale scale, SDL_Color color)
{
    int result;
    ...
    {
        case FC_HALIGN_LEFT:
            result = FC_Int_RenderLeft(font, dest, dstX, dstY, scale, fc_buffer);
            break;
        case FC_HALIGN_CENTER:
            result = FC_Int_RenderCenter(font, dest, dstX, dstY, scale, fc_buffer);
            break;
        case FC_HALIGN_RIGHT:
            result = FC_Int_RenderRight(font, dest, dstX, dstY, scale, fc_buffer);
            break;
    }
...
static FC_Rect FC_DrawBuffer(FC_Font* font, FC_Target* dest, float x, float y, 
    FC_HAlignEnum alignHorizontal, FC_VAlignEnum alignVertical, 
    FC_Scale scale, SDL_Color color)
{
    if (font == NULL) {
        SDL_Log("Error: FC_Font was NULL drawing buffer.");
        return FC_MakeRect(x, y, 0, 0);
    }
    if (dest == NULL) {
        SDL_Log("Error: dest renderer was NULL drawing buffer.");
        return FC_MakeRect(x, y, 0, 0);
    }
    return FC_DrawBufferUnsafe(font, dest, x, y, alignHorizontal, alignVertical, scale, color);
}

int FC_Int_DrawBuffer(FC_Font* font, FC_Target* dest, float x, float y, 
    FC_HAlignEnum alignHorizontal, FC_VAlignEnum alignVertical, 
    FC_Scale scale, SDL_Color color)
{
    if (font == NULL) {
    ...
    return FC_Int_DrawBufferUnsafe(font, dest, x, y, alignHorizontal, alignVertical, scale, color);
}

/* Text Cache */

/* We dont null check dest or src, but SDL handles that for us so this should be safe. */
static void FC_RenderCachedTexture(FC_Target* dest, SDL_Texture* src, 
    float x, float y, int srcWidth, int srcHeight, float xscale, float yscale, SDL_Color color)
{
    // if the src texture is full black the resulting color should be the same as the paramater
    SDL_SetTextureColorMod(src, color.r, color.g, color.b);
    // IMPORTANT: The src texture is renderered all black if the blendmode is not set.
    ...
    SDL_Rect dr = {(int)x, (int)y, (int)(xscale*srcWidth), (int)(yscale*srcHeight)};
    if (SDL_RenderCopyEx(dest, src, NULL, &dr, 0, NULL, (SDL_RendererFlip)flip)) {
        SDL_Log("Rendr copy error: %s", SDL_GetError());
    }
}

static void FC_DrawCachedTexture(FC_Target* dest, SDL_Texture* src, 
    float x, float y, int srcWidth, int srcHeight, FC_HAlignEnum hAlign, FC_VAlignEnum vAlign, 
    FC_Scale scale, SDL_Color color)
{
    // the render cached texture function does not offset for alignment for us,
    // so we do it ourselves by just changing the inputs
    int dstY = y;
    if (vAlign == FC_VALIGN_TOP) {
        // it's naturally correct for top
    } else if (vAlign == FC_VALIGN_MIDDLE) {
        dstY -= srcHeight/2.0f * scale.y;
    } else {
        dstY -= srcHeight * scale.y;
    }
    int dstX = x;
    if (hAlign == FC_HALIGN_LEFT) {
        // it's naturally correct for left
    } else if (hAlign == FC_HALIGN_CENTER) {
        dstX -= srcWidth/2.0f * scale.x;
    } else { // FC_HALIGN_RIGHT
        dstX -= srcWidth * scale.x;
    }
    FC_RenderCachedTexture(dest, src, dstX, dstY, srcWidth, srcHeight, scale.x, scale.y, color);
}

int FC_SetTextCacheSizeBytes(FC_Font* font, int bytes)
{
    if (!font) return -1;
    if (bytes < 0) {
        bytes = 0;
    ...
    font->text_cache_max_size = bytes;
    return 0;
}

int FC_SetTextCacheSize(FC_Font* font, int numTextures)
{
    if (!font) return -1;
    if (numTextures < 0) {
        numTextures = 0;
    ...
    if (numTextures > 500) {
        // that would be stupid, probably read garbage memory.
        numTextures = 50; // we'll just say 50 is a good max.
    }

    font->text_cache_max_size = numTextures;
    return 0;
}

/*
* Creates text cache from buffer text and places it into the complete cache.
* @return A pointer to the new text cache, or NULL when it was unable to cache it.
*/
static FC_TextCache* FC_CreateTextCacheFromBuffer(FC_Font* font, FC_Target* dest, FC_HAlignEnum horizontalAlign)
{
    if (!font) {
        SDL_Log("creating new text cache font param was null");
        return NULL;
    }
    if (!dest) {
        SDL_Log("creating new text cache dest param was null");
        return NULL;
    }

    FC_TextCache* newCache = NULL;
    if (font->text_cache_count >= font->text_cache_max_size) {
        if (font->text_cache_count < 1) {
            // impossible to make new cache, return NULL for fail
            return NULL;
        }
        // need to remove another cache before creating another
        int queueIndex = font->text_cache_queue_index;
        newCache = &font->text_cache[queueIndex];
        // move queue up one whenever we delete a cache, so each cache gets a fair amout of time
        font->text_cache_queue_index = (queueIndex + 1) % font->text_cache_count;

        if (newCache->texture) {
            SDL_DestroyTexture(newCache->texture);
            newCache->texture = NULL;
        } else {
            SDL_Log("FC_Error: while getting new cache from queue, texture was null");
        }
        
        font->text_cache_count--;
    } else {
        newCache = &font->text_cache[font->text_cache_count];
        newCache->texture = NULL;
    }

    newCache->srcRect = FC_GetDestinationRect(font, horizontalAlign, fc_buffer);
    if (newCache->texture != NULL) {
        FC_Log("While creating new text cache, texture was not null for new cache.");
    }
    newCache->texture = SDL_CreateTexture(dest, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_TARGET,
        newCache->srcRect.w, newCache->srcRect.h);

    newCache->hAlign = horizontalAlign;
    if (strlen(fc_buffer) < MAX_CACHED_TEXT_LENGTH) {
        strcpy(newCache->text, fc_buffer);
        font->text_cache_count++;
        return newCache;
    }

    // not enough space for texture or whatever. I think we just don't make one for it,
    // otherwise we are sacrificing other textures and gonna be destroying and creating
    // textures all the time.
    return NULL;
}

/*
* Draw the text in the buffer, while attempting to use the cached text first.
* If it does not find a cached texture, it will render a new one and cache the result.
*/
int FC_DrawBuffer_Cache(FC_Font* font, FC_Target* dest, float x, float y, 
    FC_HAlignEnum alignHorizontal, FC_VAlignEnum alignVertical, 
    FC_Scale scale, SDL_Color color)
{
    if (font == NULL) {
    ...
    // for debuggging, remove
    // set_color_for_all_caches(font, FC_MakeColor(0, 0, 0, 255));

    // check each cached text for usable caches
    for (size_t i = 0; i < font->text_cache_count; i++)
    {
        // need to make sure the actual text is identical,
        // and that the horizontal alignment is correct
        char* c = font->text_cache[i].text;
        char* b = fc_buffer;
        Uint8 strsEqual = 1;
        for (; *c != '\0'; c++,b++) {
            if (*c != *b) {
        ...
        if (strsEqual && font->text_cache[i].hAlign == alignHorizontal) {
            // found the correct cached text
            FC_TextCache textCache = font->text_cache[i];
            // move cache queue index 1 past the cache that was just used,
            // to encourage often used caches to not be removed
            font->text_cache_queue_index = (i+1) % font->text_cache_max_size;

            FC_DrawCachedTexture(dest, textCache.texture, 
                    x, y, textCache.srcRect.w, textCache.srcRect.h, 
                    alignHorizontal, alignVertical,
                    scale, color);
            // we can just return here when we find an already cached texture,
            // no need to worry about caching anything
            return 0;
        }
    }

    // If the function has gotten this far it means the cached texture
    // was unable to be used: didnt exist, or needs to be re rendered
    // try to render and cache a new one

    /* Allocate texture, then render to texture with normal render function */
    FC_TextCache* textCache = FC_CreateTextCacheFromBuffer(font, dest, alignHorizontal);
    if (textCache && textCache->texture) {
        FC_Rect srcRect = textCache->srcRect;

        SDL_SetRenderDrawBlendMode(dest, SDL_BLENDMODE_BLEND);
        SDL_SetRenderTarget(dest, textCache->texture);

        SDL_SetRenderDrawColor(dest, 255, 255, 255, 0);
        SDL_RenderClear(dest);
        
        // drawing is no longer relative to 0, 0.
        // it's relative to the texture top left
        // trying to figure out how to make it render it in the right spot
        // Thing I think is necessary: draw with no vertical alignment, add it in after
        // same with scale and color i think


        // DrawBuffer does not offset the original position, smth
        // so we do it ourselves by just changing the input x
        int dstX = x;
        if (alignHorizontal == FC_HALIGN_LEFT) {
            // do nothing
        } else if (alignHorizontal == FC_HALIGN_CENTER) {
            dstX -= (srcRect.w/2.0f); // I may not need to do this because the src rect x and y values should
            // offset it for me
        } else { // FC_HALIGN_RIGHT
            dstX -= srcRect.w;
        }
        SDL_Log("before draw buffer");
        SDL_SetTextureBlendMode(textCache->texture, SDL_BLENDMODE_BLEND);
        FC_DrawBuffer(font, dest, -srcRect.x / 2.0f, 0, alignHorizontal, FC_VALIGN_TOP, FC_MakeScale(1,1), color);
        SDL_Log("after draw buffer");

        // SDL_SetRenderDrawColor(dest, 255, 0, 0, 255);
        // SDL_RenderDrawLine(dest, 0,0,1000,1000);
        // SDL_RenderDrawRect(dest, &textCache->srcRect);

        SDL_SetRenderTarget(dest, NULL);

        SDL_Log("before rendering cached texture after making it");
        FC_DrawCachedTexture(dest, textCache->texture, 
            x, y, srcRect.w, srcRect.h, 
            alignHorizontal, alignVertical,
            scale, color);

        SDL_Log("after rendering cached texture immediately after creating it");

    } else {
        // couldn't make new cache for some reason, just render normally, no cache
        SDL_Log("drawing from scratch and not caching");
        FC_Int_DrawBufferUnsafe(font, dest, x, y, alignHorizontal, alignVertical, scale, color);
    }
    
    return 0;
}

// shorten things needed to do on all var args draw calls
#define _FC_EXTRACT_DRAW(buffer, formatted_text) {             \
    if (formatted_text == NULL) {                              \
        SDL_Log("Error: text passed to FC draw call was null");\
        return;                        \
    }                                                          \
    FC_EXTRACT_VARARGS(buffer, formatted_text);                \
}                                                              \

void FC_DrawAlignVS(FC_Font* font, FC_Target* dest, float x, float y, 
    FC_HAlignEnum alignHorizontal, FC_VAlignEnum alignVertical, FC_Scale scale,
    const char* formatted_text, ...)
{
    _FC_EXTRACT_DRAW(fc_buffer, formatted_text);
    FC_Int_DrawBuffer(font, dest, x, y, alignHorizontal, alignVertical, scale, font->default_color);
}

void FC_DrawAlignV(FC_Font* font, FC_Target* dest, float x, float y, 
    FC_HAlignEnum alignHorizontal, FC_VAlignEnum alignVertical, 
    const char* formatted_text, ...)
{
    _FC_EXTRACT_DRAW(fc_buffer, formatted_text);
    FC_Int_DrawBuffer(font, dest, x, y, alignHorizontal, alignVertical, FC_MakeScale(1,1), font->default_color);
}

void FC_DrawScale(FC_Font* font, FC_Target* dest, float x, float y, float scale, const char* formatted_text, ...)
{
    if (font == NULL || dest == NULL) return;
    _FC_EXTRACT_DRAW(fc_buffer, formatted_text);
    set_color_for_all_caches(font, font->default_color);
    FC_Int_RenderLeft(font, dest, x, y, FC_MakeScale(scale, scale), fc_buffer);
}

void FC_DrawScaleXY(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* formatted_text, ...)
{
    if (font == NULL || dest == NULL) return;
    _FC_EXTRACT_DRAW(fc_buffer, formatted_text);
    set_color_for_all_caches(font, font->default_color);
    FC_Int_RenderLeft(font, dest, x, y, scale, fc_buffer);
}

void FC_DrawAlign(FC_Font* font, FC_Target* dest, float x, float y, FC_HAlignEnum h_align, const char* formatted_text, ...)
{
    _FC_EXTRACT_DRAW(fc_buffer, formatted_text);
    FC_Int_DrawBuffer(font, dest, x, y, h_align, FC_VALIGN_TOP, FC_MakeScale(1, 1), font->default_color);
}

void FC_DrawColor(FC_Font* font, FC_Target* dest, float x, float y, SDL_Color color, const char* formatted_text, ...)
{
    if (font == NULL || dest == NULL) return;
    _FC_EXTRACT_DRAW(fc_buffer, formatted_text);
    set_color_for_all_caches(font, color);
    FC_Int_RenderLeft(font, dest, x, y, FC_MakeScale(1,1), fc_buffer);
}

void FC_DrawEffect(FC_Font* font, FC_Target* dest, float x, float y, FC_Effect effect, const char* formatted_text, ...)
{
    _FC_EXTRACT_DRAW(fc_buffer, formatted_text);
    FC_Int_DrawBuffer(font, dest, x, y, effect.horizontal_align, effect.vertical_align, effect.scale, effect.color);
}

void FC_DrawAny(FC_Font* font, FC_Target* dest, float x, float y, 
    FC_HAlignEnum alignHorizontal, FC_VAlignEnum alignVertical,
    FC_Scale scale, SDL_Color color,
    const char* formatted_text, ...)
{
    _FC_EXTRACT_DRAW(fc_buffer, formatted_text);
    FC_Int_DrawBuffer(font, dest, x, y, alignHorizontal, alignVertical, scale, color);
}

/* draw using text caching */
void FC_DrawCached(FC_Font* font, FC_Target* dest, float x, float y, 
    FC_HAlignEnum alignHorizontal, FC_VAlignEnum alignVertical,
    FC_Scale scale, SDL_Color color,
    const char* formatted_text, ...)
{
    _FC_EXTRACT_DRAW(fc_buffer, formatted_text);
    FC_DrawBuffer_Cache(font, dest, x, y, alignHorizontal, alignVertical, scale, color);
}

/* Possible TODO: draw caching priority system */